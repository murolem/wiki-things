-- Utilites and common functions to use in bucket modules.
...
---Check whether string starts with substring. Case sensitive.
---@param str string String to check.
---@param substr string Substring to check for.
---@return boolean Result Result.
function p.startsWith(str, substr)
    return string.sub(str, 1, string.len(substr)) == substr
end

---Check whether string ends with substring. Case sensitive.
---@param str string String to check.
---@param substr string Substring to check for.
---@return boolean Result Result.
function p.endsWith(str, substr)
    local substr_length = string.len(substr)
    return string.sub(str, string.len(str) - substr_length + 1, string.len(str)) == substr
end

---Check whether string starts with substring. Case insensitive.
---@param str string String to check.
---@param substr string Substring to check for.
---@return boolean Result Result.
function p.startsWithInsensitive(str, substr)
    return startsWith(string.lower(str), string.lower(substr))
end

---Check whether string ends with substring. Case insensitive.
---@param str string String to check.
---@param substr string Substring to check for.
---@return boolean Result Result.
function p.endsWithInsensitive(str, substr)
    return endsWith(string.lower(str), string.lower(substr))
end
...
---Format sound arguments in `rowTbl` and assign them to resulting table `resTbl` to be rendered via an infobox.
---Goes through all of them as long as they are consecutive.
---@param frame table Frame.
---@param rowTbl table Input arguments table. Sound fields are expected to be named like `soundN` with optional `soundNcaption`, eg `sound2` + `sound2caption`.
---The fields are expected to be defined consecutively, starting with 1. If no fields are found, nothing is done.
---@param resTbl table Output arguments table. The result will be assigned to this table to be passed to an infobox.
function p.formatSounds(frame, rowTbl, resTbl)
    local idx = 1
    while true do
        ...
        local caption = rowTbl["sound" .. idx .. "caption"]
        if caption then
            wrapper:wikitext("&nbsp;" .. caption)
        end

        resTbl["sound" .. idx] = tostring(wrapper)
        resTbl["sound" .. idx .. "_wide"] = "yes"
        idx = idx + 1
    end
end

---Format quality argument in `rowTbl` and assign the result to resulting table `resTbl` to be rendered via an infobox.
---@param frame table Frame.
---@param rowTbl table Input arguments table.
---@param resTbl table Output arguments table. The result will be assigned to this table to be passed to an infobox.
---@param infoboxType string Type of the infobox. Used in text. Examples: item, liquid.
function p.formatQualities(frame, rowTbl, resTbl, infoboxType)
    if not resTbl.qualities then return end

    -- maps qualities to extra categories.
    local qualityLcToCategoryMap = {
        cutting = "[[Category:Tools]]",
        hammering = "[[Category:Tools]]",
    }

    local caption = string.format("{{ui icon|quality|{{ui tooltip|Qualities|Specific characteristics of this %s.}}}}", infoboxType)

    resTbl.qualities = tostring(p.listToTableEl {
        caption = frame:preprocess(caption),
        headers = { "Quality", "Count" },
        rows = rowTbl.qualities,
        process = function(args)
            -- column: Quality
            if args.column == 1 then
                local label = p.capitalizeFirst(args.value)
