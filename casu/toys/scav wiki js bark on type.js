(() => {
    class AudioController {
        ...
        /** Maps audio urls to audio objects.
         * @type {Map<string, Set<{ audio: Audio, loaded: boolean, free: boolean }>>}  */
        #poolBusy = new Map();
        /** Maps audio urls to audio objects.
         * @type {Map<string, Set<{ audio: Audio, loaded: boolean, free: boolean }>>}   */
        #poolFree = new Map();

        constructor(maxSoundsPerSecond, maxSounds) {
        ...
        /**
         * Ensures that a group with given url exists for the given pool and returns it.
         * @param {Map} pool 
         * @param {string} url 
         */
        #ensurePoolGroup(pool, url) {
        ...
        /**
         * Creates a new worker fro the given url.
         * @param {string} url Url to create the worker for.
         */
        #createWorker(url) {
            url = sanitizeUrl(url);
         ...
         * @param {string} url Url to get the worker for.
         */
        #getFreeWorker(url) {
            url = sanitizeUrl(url);

            const groupFree = this.#poolFree.get(url);
            const groupBusy = this.#poolBusy.get(url);
            if(!groupFree || groupFree.size === 0) {
                const newWorker = this.#createWorker(url);
                const 
            }
        ...
        load(url) {
            ...
            let group = this.#poolBusy.get(url);
            if (!group) {
                group = new Set();
            }
            ...
            return new Promise(resolve => {
                audio.addEventListener('canplaythrough', () => resolve([audio, () => this.play(url)]));
            })
            return [audio, this.play(url)];
        }

        play(urlOrAudio) {
            if (this.#playing.length > this.#maxSounds)
                return;
            else if (Date.now() < this.#nextPlayTs)
                return;

            const audio = typeof urlOrAudio === 'string'
                ? new Audio(urlOrAudio)
                : urlOrAudio;
            const onEnded = () => {
            ...
            this.#nextPlayTs = Date.now() + (1000 / this.#maxSoundsPerSecond);
        }
    }

    class TempClassController {
        static #store = []

        static schedule(el, className, duration, delay, opts = {}) {
            delay ??= 1;
            opts.onEndCb ??= () => { };

            const ts = Date.now()

            let item = TempClassController.#store.find(item => item.el === el && item.className === className);
            if (!item) {
                const expiresFn = () => {
                    el.classList.remove(className);
                    TempClassController.#store.splice(TempClassController.#store.indexOf(item, 1));
                    opts.onEndCb();
                }
                ...
                TempClassController.#store.push(item);
            } else {
    ...
    const audioCtrl = new AudioController(20, 15);

    window.addEventListener("keydown", e => {
        if (e.key === "Backspace") {
            audioCtrl.play("https://static.wikitide.net/casualtiesunknownwiki/3/31/Lobotomy.ogg");
            ...
            // TempClassController.schedule(document.documentElement, "has-overlay", overlayDuration, 1, { onEndCb: TempClassController.schedule(document.documentElement, "fade-in", overlayDuration, 2) });
            document.body.append(overlay);
        } else {
            audioCtrl.play("https://static.wikitide.net/casualtiesunknownwiki/f/f2/Bark_sample.ogg")

            const editor = document.querySelector('.wikiEditor-ui');
            TempClassController.schedule(editor, "jump", 150);
        }
    });
...
.overlay {
    ...
    transition: 1s background-color 2s ease-out;
...
.jump {
    animation: jump 150ms cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards;
}
...
html.has-overlay {
    ...
    &.fade-in {
        transform: scale(1);
        transition: .8s transform 2s ease-out;
    }
}
